...
| P15 | The VMT single-fare comparison must use a maintained table, not a manual per-case lookup. | — | `/vergleichsrechnung` (Admin) lists every TN whose month triggers the comparison and lets an Admin maintain the VMT single-fare table (`VmtFaresProvider`) that feeds it — editing a fare recomputes and clears the blocker immediately. | `src/features/admin/Vergleichsrechnung.tsx`, `src/domain/vmtFares.ts`, `src/domain/vergleichsrechnung.ts`, `src/app/vmt-fares-context.tsx`, `src/domain/reimbursement.ts`, `src/domain/__tests__/vergleichsrechnung.test.ts`, `src/domain/__tests__/vmtFares.test.ts`, `src/domain/__tests__/reimbursement.test.ts` |
