...
```bash
npm install
npm test        # 158 tests across 25 files — must stay green
...
names, unrelated to the demo workbook roster). The seed's names are checked
against the demo workbooks by `no-real-names.test.ts` (see "Demo data"
below), and switching roles changes what the application state contains,
not just what's rendered. Privacy by design, not by CSS. Covered by
...
- **Per cohort-year** (formula — plug in the actual cohort size; the demo
  seed has 15 example participants, not a real cohort count): cohort size ×
...
`src/adapters/mock/seedData.ts` is **generated**, never hand-edited, from
exactly two committed workbooks in `public/demo/`:

- `Testdaten_Alle_TN_Daten.xlsx` — Stammdaten (master data) + monthly
  Übersicht, 15 fictional participants.
- `Testdaten_Anwesenheitsliste.xlsx` — day-level attendance marks,
  Jahresblatt layout (one tab per year).
...
`scripts/build-seed.ts` reads both files through the same adapters the app
uses for a real upload (`ExcelWorkbookSource`, `LocalYearWorkbook`) and
writes `src/adapters/mock/seedData.ts` — so the seed has exactly the shape a
live-loaded workbook would produce, no invented fields. A field the
workbooks don't populate (there currently are none left blank) stays blank
rather than being filled in by the generator.

**A data refresh must always go through these two workbooks and
`seed:build` — never from a real export.** `src/domain/__tests__/no-real-names.test.ts`
enforces this going forward: every name in the generated seed must trace
back to one of the two workbooks, checked positively rather than against a
blocklist. See `docs/DECISIONS.md` for the incident that made this
necessary: an earlier version of this seed was generated from a real Excel
export and carried real participant data.
...
or to the demo workbook roster.

For the review build's Manager task specifically, three of the 15
